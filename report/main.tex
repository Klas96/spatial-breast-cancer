...
The tumour microenvironment (TME) in breast cancer is a complex ecosystem of malignant, stromal, and immune cells whose spatial organisation has profound implications for tumour progression, immune evasion, and therapeutic response. Recent advances in spatial transcriptomics—particularly the 10x Genomics Visium platform—enable simultaneous profiling of the transcriptome and tissue architecture at near-single-cell resolution. Here we present an end-to-end computational pipeline that integrates standard bioinformatics approaches with a graph convolutional network (GCN) to characterise the spatial TME in human breast cancer. The pipeline encompasses quality control, normalisation, dimensionality reduction, Leiden clustering, Bayesian cell-type deconvolution via Cell2location, spatially variable gene (SVG) detection using Moran's $I$, cell-type co-localisation analysis, and finally a two-layer GCN that predicts tissue domain labels directly from the spatial gene expression graph. All analyses are fully reproducible via a \texttt{uv}-managed Python environment. Code is available at \url{https://github.com/Klas96/spatial-breast-cancer}.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        \multicolumn{5}{c}{\textit{(Results populated after pipeline completion)}} \\
        \midrule
        Macro avg & -- & -- & -- & -- \\
        \bottomrule
    \end{tabular}
...
\paragraph{GCN for spatial domain prediction.} Using dominant cell type as a proxy for tissue domain is a reasonable first approximation, but tissue domains in cancer are heterogeneous. The GCN formulation presented here—with PCA embeddings as node features and spatial adjacency as edges—closely mirrors graph-based approaches used in recent spatial omics studies. Extensions could include multi-relational graphs (combining transcriptional and spatial edges), attention-based graph convolutions (GAT), or unsupervised graph clustering.
